...
export const connectHttpTransport = (port: number) => {
  ...
  app.post('/mcp', async (req, res) => {
    ...
    if (sessionIdFromHeader && httpTransports[sessionIdFromHeader]) {
      ...
      const argocdBaseUrl = (req.headers['x-argocd-base-url'] as string) || process.env.ARGOCD_BASE_URL || '';
      const argocdApiToken = (req.headers['x-argocd-api-token'] as string) || process.env.ARGOCD_API_TOKEN || '';
